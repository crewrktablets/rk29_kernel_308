...
const init_mma_list_t init_mma[] = {
		{ MMA8452_REG_F_SETUP,		0 },	/* disable FIFO  FMODE = 0*/
		{ MMA8452_REG_XYZ_DATA_CFG,	0 },	/* set full scale range to 2g */
		{ MMA8452_REG_CTRL_REG3,	5 },
		{ MMA8452_REG_CTRL_REG4,	1 },
		{ MMA8452_REG_CTRL_REG5,	1 },
		{ 0, 0 }
};
...
static int mma845x_read_reg( struct i2c_client *client, int addr, unsigned char *val)
{
	int ret;
	*val = addr;
	return mma8452_rx_data(client, val, 1);
}
...
{
	unsigned char id;
	int ret = mma845x_read_reg( client, MMA8452_REG_WHO_AM_I, &id);
	if (ret) {
		dev_info( &client->dev, "Error %d reading ID\n", ret);
		return 0;
	}
	else {
		if ((id & 0x0f) == 0x0a)
		dev_info( &client->dev, "mma845%x recoginzed\n", id >> 4);
	}
	return id;
}
...
static int mma845x_active(struct i2c_client *client,int enable)
{
	...
	ret = mma845x_read_reg(client,MMA8452_REG_CTRL_REG1, &tmp);
	if (ret)
		goto error_out;
	...
	return mma845x_write_reg(client,MMA8452_REG_CTRL_REG1,tmp);
...
{
	struct mma8452_data *mma8452 = (struct mma8452_data *)i2c_get_clientdata(client);   // mma8452_data ������ mma8452.h ��.
	const init_mma_list_t *p = init_mma;
	int ret = 0;
	int tmp;
	...
	/* spool init values into mma8452 */
	while( p->reg && (ret == 0)) {
		ret = mma845x_write_reg( client, p->reg, p->val);
		p++;
	}
	if (ret) {
		dev_err( &client->dev, "Error %d starting device\n", ret);
		goto error_out;
	}

	/* set sample rate of chip */
...
static long mma8452_ioctl( struct file *file, unsigned int cmd,unsigned long arg)
{
    ...
    struct mma8452_data* this = (struct mma8452_data *)i2c_get_clientdata(client);  /* �豸���ʵ���ָ��. */
...
static int mma8452_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	mmaprintkd("%s enter 2 level suspend mma8452->status %d\n", __func__, mma8452->status);
...
static int mma8452_resume(struct i2c_client *client)
{
	...
	mmaprintkd("%s 2 level resume mma8452->status %d\n", __func__, mma8452->status);
...
static int mma8452_init_client(struct i2c_client *client)
{
	...
	mmaprintk("%s with GPIO %u at IRQ %u\n", __func__, client->irq, gpio_to_irq(client->irq));
    ...
    if (ret) {
        mmaprintk("failed to set mma8452_int GPIO gpio input\n");
