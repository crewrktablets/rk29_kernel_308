...
#error OBSOLETE DRIVER

#if defined (CONFIG_MACH_ODYS_XPRESS)

/* ADC setup */
#define ADC_REF_VALUE			2500	/* Voltage of internal reference 2.5V */
...
#define BATT_ZERO_VOL_VALUE		7000	/* Minimum level equals 0% (6.700V) */
#define BATT_NOMAL_VOL_VALUE	7450	/* No idea */
#define SHUTDOWNVOLTAGE			6700	/* Absolute shutdown voltage */

#elif defined (CONFIG_MACH_ODYS_LOOX)
#define ADC_REF_VALUE	        3000	/* Voltage of internal reference 3.0V */
#define BAT_PULL_UP_R           200		/* ADC resistor network */
#define BAT_PULL_DOWN_R		     200

#define BATT_MAX_VOL_VALUE	    3700	/* Maximum level equals 100% (3.7V) */
#define BATT_ZERO_VOL_VALUE    3000		/* Minimum level equals 0% (3.0V) */
#define BATT_NOMAL_VOL_VALUE   3800		/* No idea */
#define SHUTDOWNVOLTAGE			4100	/* Absolute shutdown voltage */

#endif

#define	 TIMER_MS_COUNTS				 50	/**< ADC sample period in ms, base time for work queue. */
//���²�����Ҫ���ʵ�ʲ��Ե���
...
#define CHARGE_MAX_SECOND	            150	//�����1%ʱ��
#define CHARGE_FULL_DELAY_TIMES         10	//����������ʱ��
#define USBCHARGE_IDENTIFY_TIMES         5	//����USB������pcʶ����ʱ��
#define	 SPEEDLOSE_TIME					 10 /* AX: I have no idea what this might be good for */
#define SHUTDOWN_TIME					 10 /* AX: I have no idea what this might be good for */
...
#define NUM_SPEEDLOSE_SAMPLE			((SPEEDLOSE_TIME * 1000) / TIMER_MS_COUNTS)
#define NUM_SHUTD0WN_SAMPLE				((SHUTDOWN_TIME * 1000) / TIMER_MS_COUNTS)

int rk29_battery_dbg_level = 0;

#define adc_to_voltage(adc_val) ((adc_val * ADC_REF_VALUE * (BAT_PULL_UP_R + BAT_PULL_DOWN_R)) / (1024 * BAT_PULL_DOWN_R))
...
#elif defined(CONFIG_MACH_M1005HN) || defined(CONFIG_MACH_M1005HNW) || defined(CONFIG_MACH_M1005HD) \
    || defined(CONFIG_MACH_M1006HD) || defined(CONFIG_MACH_M1007HD) || defined(CONFIG_MACH_ODYS_XPRESS)
	7000, 7286, 7405, 7453, 7525, 7606, 7710, 7855, 7977, 8115, 8220
#elif defined (CONFIG_MACH_M727) || defined (CONFIG_MACH_M722HCN) || defined (CONFIG_MACH_M727HCN) \
	|| defined (CONFIG_MACH_M723HR)
...
{
#if defined(CONFIG_MACH_RK29_ACH7)
	//3760, 3886, 3964, 3989, 4020, 4062, 4123, 4180, 4189, 4190, 4190      //ʵ�ʲ���ֵ
...
static int gBatLastChargeCapacity = 100;          //��¼���ʱ�ĵ������
...
static int *pSamples = &gBatVoltageSamples[0];		//�����ָ��
static int gFlagLoop = 0;		//�����㹻��־
...
static uint8_t openfailcount = 20;  //  �ļ���ʧ�ܵĻ����ظ�����
...
static int rk2918_battery_resume_get_Capacity(int deltatime)
{
    ...
    if ((gBatChargeStatus == 1) && (last_BatChargeStatus == 1))
    {
        ...
        if (capacity > gBatCapacity + (deltatime / CHARGE_MIN_SECOND))       //����������ƫ��ϴ󣬽���������
        {
    ...
    else
    {
        ...
        if (capacity < gBatCapacity - (deltatime / DISCHARGE_MIN_SECOND))    //����������ƫ��ϴ󣬽���������
        {
...
static void rk2918_get_bat_capacity(struct rk2918_battery_data *bat)
{
	...
	else if ((deltatime > 1) && (first_flag == 0) &&(!time_chg_flag))//��������֮��ĵ����ظ�,����ʮ����
	{	
