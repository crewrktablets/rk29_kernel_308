...
static int mmc_rescan_try_freq(struct mmc_host *host, unsigned freq)
{
    ...
    if( strncmp( mmc_hostname(host) ,"mmc0" , strlen("mmc0")) )
    {
		/* Card is registered as something different, not mmc/sd */
    	...
    	if (err == 0) {
			printk("Init SDIO %s OK\n", mmc_hostname(host));
			goto out_good;
    	}
    	else if( err != 0xFF) {
			printk("\n %s:%d INIT ERROR %d SDIO %s\n", __FUNCTION__,  __LINE__, err, mmc_hostname(host));
			goto freq_out;
		}
    }
    else
    ...
    /* Check for inserted or attached SDIO */
	err = mmc_attach_sdio(host);
	if (err == 0) {
	    printk("Init MMC %s OK\n", mmc_hostname(host));
		goto out_good;
	}
	else if( err != 0xFF)
	{
		printk("\n %s:%d INIT ERROR %d MMC %s\n", __FUNCTION__,  __LINE__, err, mmc_hostname(host));
		goto freq_out;
	}
	...
	if (err == 0) {
	    printk("Init SDCARD %s OK\n", mmc_hostname(host));
		goto out_good;
	}
	else if( err != 0xFF) {
		printk("\n %s:%d INIT ERROR SDCARD %s\n", __FUNCTION__,  __LINE__, mmc_hostname(host));
		goto freq_out;
	}

    /* Check for inserted MMC or attached eMMC */
...
{
// Astralix	static const unsigned freqs[] = { 400000, 300000, 200000, 100000 };
