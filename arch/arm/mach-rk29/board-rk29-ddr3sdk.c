...
/* Astralix:
 *
 * Neat collection of used pins and GPIOS... Please keep it up to date!
 *
 * WLAN on xpress:
 ...
 * #define TOUCH_SCREEN_DISPLAY_PIN         RK29_PIN6_PD2
...
/* Astralix:
 *
 * Loox has only front CAM OV7675 at bus 1 address 0x42, but
 * driver needs two for correct init.
 * So just choose a second CAM and set its I2C address to 0.
 *
 * Xpress has two CAMs, rear is OV7675 at bus 1 address 0x42,
...
/*---------------- Camera Sensor Configuration Macro Begin ------------------------*/
#if defined (ODYS_LOOX)
#ifdef CONFIG_SOC_CAMERA_OV7675			/* front camera sensor */
	#warning "***** SOC CAM OV7675 DEFINED *****"
	...
	#warning "***** SOC CAM OV2655 DEFINED *****"
	...
	#define CONFIG_SENSOR_0 RK29_CAM_SENSOR_OV5642                      /* rear camera sensor */
	...
	#warning "***** SOC CAM OV2659 DEFINED *****"
...
static int i2c_lz300_init_platform_hw(void)
{
	if(gpio_request(TOUCH_RESET_PIN,NULL) != 0){
		gpio_free(TOUCH_RESET_PIN);
		printk("i2c_lz300_init_platform_hw gpio_request error\n");
		return -EIO;
	}
...
#endif
// Astralix: Skip this, as it is the wrong one.
